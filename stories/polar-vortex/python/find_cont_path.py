...
def smooth_endpoint_connection(trajectory):
    ...
    # Adjust latitude of last few points to create smooth connection
    n_points = 5  # Number of points to adjust
    ...
    if east_idx >= n_points and west_idx + n_points < len(trajectory):
        # Create smooth transition in latitude
...
# Apply smoothing to best trajectory
best_traj = smooth_endpoint_connection(trajectories[best_temp_idx])

# Plot the smoothed best temperature-following trajectory
if len(best_traj) > 1:
    ax.plot(best_traj[:, 0], best_traj[:, 1], '-', 
            color='white',  # Changed to white
            linewidth=12,   # Changed to 12
            transform=ccrs.PlateCarree(),
            zorder=10)
